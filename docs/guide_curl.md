...
```
export LOOSE_TOUCH_ID_TOKEN_FOR_USER_1=` curl "http://127.0.0.1:8080/v1/login/google?googleIdToken=${GOOGLE_ID_TOKEN_FOR_USER_1}&googleAccessToken=${GOOGLE_ACCESS_TOKEN_FOR_USER_1}" -X GET -s | jq -r '.idToken'`
```
## Get profile information.
```
curl "http://127.0.0.1:8080/v1/profile" -H "Authorization: Bearer ${LOOSE_TOUCH_ID_TOKEN_FOR_USER_1}" -X GET -s | jq
```